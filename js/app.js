class RFIDBluetoothMonitor {
  constructor() {
    ...
    this.SERVICE_UUID = "0000ffe0-0000-1000-8000-00805f9b34fb"; // Стандартный сервис для данных
    this.CHARACTERISTIC_UUID = "0000ffe1-0000-1000-8000-00805f9b34fb"; // Стандартная характеристика для данных
  ...
  async checkBluetoothAvailability() {
    ...
    try {
      ...
      if (!availability) {
        this.addLog(
          "Bluetooth адаптер не доступен. Убедитесь, что Bluetooth включен.",
          "error"
        );
      } else {
  ...
  async connect() {
    try {
      ...
      this.addLog(
        `Устройство выбрано: ${this.device.name || "Unknown"}`,
        "success"
      );
      ...
      this.updateConnectionStatus(
        "connected",
        `Подключено: ${this.device.name || "Unknown"}`
      );
